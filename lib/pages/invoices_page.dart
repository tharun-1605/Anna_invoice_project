...
class _InvoicesPageState extends State<InvoicesPage> {
  ...
  Future<void> _bulkDelete() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete ${_selectedIds.length} invoices? This cannot be undone.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _exportCsv() =>
      CsvExporter.exportInvoices(context, widget.invoices);
  ...
  Widget build(BuildContext context) {
    ...
    final filtered =
        widget.invoices.where((inv) {
          ...
          if (_selectedYear != 'All' &&
              inv.date.year.toString() != _selectedYear) {
            return false;
          }
          ...
          if (_selectedStatus != 'All' &&
              _invoiceStatus(inv) != _selectedStatus) {
            return false;
          }
    ...
    final filteredBalance = filtered.fold<double>(
      0,
      (sum, inv) => sum + inv.due,
    );
    final years = [
      'All',
      ...widget.invoices.map((e) => e.date.year.toString()).toSet().toList()
        ..sort(),
    ];
...
class InvoiceActionRow extends StatelessWidget {
  ...
  Future<void> _managePayments(BuildContext context) async {
    await showDialog<void>(
      context: context,
      builder: (context) =>
          ManagePaymentsDialog(invoice: invoice, store: store),
    );
  }
  ...
  Future<void> _sendReminder(BuildContext context) async {
    final text =
        'Hello ${invoice.client.name}, this is a reminder that Invoice ${invoice.number} for ${money.format(invoice.total)} is due on ${dateFormatter.format(invoice.dueDate)}. The current balance is ${money.format(invoice.due)}. Please arrange payment. Thank you!';
...
class _ManagePaymentsDialogState extends State<ManagePaymentsDialog> {
  ...
  void _addPayment() {
    final currentDue =
        widget.invoice.total - payments.fold(0.0, (s, p) => s + p.amount);
    if (currentDue <= 0) return;
    setState(() {
      payments.add(
        Payment(amount: currentDue, method: 'Cash', date: DateTime.now()),
      );
    });
  }
            ...
            children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('Installment ${i + 1}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black87)),
                          IconButton(
                            icon: const Icon(Icons.delete, color: Colors.red),
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items:
                            {
                                  ...methods,
                                  if (!methods.contains(payments[i].method))
                                    payments[i].method,
                                }
                                .map(
                                  (m) => DropdownMenuItem(
              ...
              if (currentDue > 0)
                OutlinedButton.icon(
                  ...
                  label: Text(
                    'Add Installment (Due: ${currentDue.toStringAsFixed(2)})',
                  ),
                ),
